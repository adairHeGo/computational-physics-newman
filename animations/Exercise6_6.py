from numpy import zeros,empty, arange, array
...
masses = array([vp.sphere(pos=vp.vec(2*i, 0, 0), radius=0.2) for i in range(N)])

for t in arange(0, 100*pi, 0.1):
    vp.rate(30)
    delta_x = cos(omega*t)
    for i in range(N):
        masses[i].pos = vp.vec(2*i + x[i]*delta_x,0,0) 
